...
export default function SessionsPage() {
  ...
  // View mode + anchor date
  const [mode, setMode] = useState('week'); // 'day' | 'week' | 'month'
  const [anchor, setAnchor] = useState(() => startOfWeek(new Date()));

  // Filters
  const [q, setQ] = useState('');                 // class name contains
  ...
  // Build filter option lists from fetched data
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => {
            setAnchor(a => mode === 'day' ? addDays(a, -1) : mode === 'week' ? addDays(a, -7) : addMonths(a, -1));
          }}>{'‹'}</Button>
          ...
          <Button variant="ghost" onClick={() => {
            setAnchor(mode === 'week' ? startOfWeek(new Date()) : mode === 'day' ? startOfDay(new Date()) : startOfMonth(new Date()));
          }}>Today</Button>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Select value={mode} onValueChange={(v) => {
            setMode(v);
            setAnchor(a => v === 'day' ? startOfDay(a) : v === 'week' ? startOfWeek(a) : startOfMonth(a));
          }}>
      ...
      <Card>
        <CardContent className="pt-4">
          <div className="grid md:grid-cols-5 gap-3 items-end">
            ...
            <div className="flex items-center gap-2">
              <Switch id="bookable-switch" checked={onlyBookable} onCheckedChange={setOnlyBookable} />
              <Label htmlFor="bookable-switch" className="cursor-pointer">Bookable only</Label>
            </div>

            <div className="flex items-center gap-2">
              <Switch id="spots-switch" checked={onlySpots} onCheckedChange={setOnlySpots} />
